...
from saab_suite.kernel.errors import FlashAbortedError
...
def execute(
    ...
    mismatch raises FlashAbortedError before any I/O is performed.

    Raises:
        FlashAbortedError: token mismatch, kill switch unset, lock contention.
    """
    if confirm_token != validated.confirm_token:
        msg = "confirm token does not match validated plan"
        raise FlashAbortedError(msg)
